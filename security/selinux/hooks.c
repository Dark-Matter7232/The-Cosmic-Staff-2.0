			       ...
			       int cap, int audit, bool initns)
{
	...
	rc = avc_has_perm_noaudit(&selinux_state, sid, sid, sclass, av, 0, &avd);
				 ...
				 u32 *_new_isid)
{
	...
	if ((sbsec->flags & SE_SBINITIALIZED) &&
		...
		*_new_isid = tsec->create_sid;
		...
		return security_transition_sid(&selinux_state, tsec->sid, dsec->sid, tclass, name, _new_isid);
	}
...
// ] SEC_SELINUX_PORTING_COMMON
	...
	rc = avc_has_perm_noaudit(&selinux_state, sid, isec->sid, isec->sclass, perms, 0, &avd);
				  ...
				  const void *value, size_t size, int flags)
{
	...
	rc = security_context_to_sid(&selinux_state, value, size, &newsid, GFP_KERNEL);
	if (rc == -EINVAL) {
	...
	rc = security_validate_transition(&selinux_state, isec->sid, newsid, sid,
					  isec->sclass);
	if (rc)
		return rc;
